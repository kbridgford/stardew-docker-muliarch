...
vanilla Linux game into `src/steam`. SteamCMD prompts for authentication on the
host; **builds never download Steam/game files or accept Steam credentials**.
An existing cache is reused. Updates require `./pullValleyBin.sh --refresh`.

Use `./scripts/podman-steam.sh build v3x86` for a cache-locked rootless build.
See [local development](../docs/local-development.md) for private VNC settings,
run/smoke commands, persistence, other targets, and ARM emulation setup.
The Steam Compose file retains its local build context with shared named inputs; do not
refresh the cache concurrently with unmanaged Compose builds.
### GOG
