...
usage() {
    printf '%s\n' \
        ...
        'Targets: v3x86, v3arm-amd64 (vanilla), v4x86-x11vnc, v3arm64, all' \
        'Options: --env-file FILE --web-port 5801 --vnc-port 5902 --game-port 24642' \
        '         --timeout 180 --lan (game UDP only) --cid-file FILE (retain created ID)' \
        '         --platform linux/amd64|linux/arm64 (v3arm64 run/smoke/doctor; default arm64)' \
        'Building v3arm64 uses Podman buildx for both platforms in one local manifest.' \
...
if [[ "$target" == all ]]; then
    [[ "$ACTION" != run ]] || steam_die 'Use smoke all sequentially, or run one target with explicit ports.'
    failures=()
    for target in "${STEAM_TARGETS[@]}"; do
        if bash "${BASH_SOURCE[0]}" "$ACTION" "$target" "${options[@]}"; then
            printf '%s: %s completed.\n' "$target" "$ACTION"
        else
            failures+=("$target")
        fi
    done
    (( ${#failures[@]} == 0 )) || steam_die "Failed/blocked targets: ${failures[*]}"
    exit
fi
steam_require_tools
select_target "$target"
ENV_FILE='' CID_FILE='' RUNTIME_PLATFORM='' WEB_PORT=5801 VNC_PORT=5902 GAME_PORT=24642 STARTUP_TIMEOUT=180 GAME_BIND=127.0.0.1
while (( $# )); do
    case "$1" in
        --env-file|--cid-file|--platform|--web-port|--vnc-port|--game-port|--timeout)
            ...
            shift 2 ;;
        --lan) GAME_BIND=0.0.0.0; shift ;;
        *) steam_die "Unknown option: $1" ;;
    esac
done
if [[ -n "$RUNTIME_PLATFORM" ]]; then
    [[ "$TARGET" == v3arm64 && "$ACTION" =~ ^(run|smoke|doctor)$ ]] ||
        steam_die '--platform is supported only for v3arm64 run, smoke or doctor; its build always includes both platforms.'
...
case "$ACTION" in
    ...
    build)
        ...
        if [[ "$TARGET" == v3arm64 ]]; then
            ...
            podman build --platform "linux/$ARCH" --tag "$IMAGE" "${build_args[@]}"
            verify_runtime_image
        fi
        ;;
    logs|stop)
